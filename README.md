...
1. `KanbanChat-1.0.1.jar` を `plugins/` に置いてサーバーを再起動します。
...
```bash
./deploy.sh        # Mac ネイティブ（JDK 25 + Maven）。生成物: target/KanbanChat-1.0.1.jar
# または
mvn -B clean package
```
...
[ビルド](#ビルド) の手順で `target/KanbanChat-1.0.1.jar` を生成します。
...
docker cp target/KanbanChat-1.0.1.jar <コンテナ名>:/data/plugins/
docker restart <コンテナ名>
```
...
`PLUGINS` は改行区切りで複数指定できます。バージョンを更新したら、URL の `v1.0.1` とファイル名を新しいリリースに合わせて変更してください（例: `.../download/v1.0.1/KanbanChat-1.0.1.jar`）。
